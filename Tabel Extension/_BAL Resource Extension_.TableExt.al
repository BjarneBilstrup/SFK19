tableextension 50101 "BAL Resource Extension" extends resource
{
    fields
    {
        ...
        field(50103; "BAL Archive Quote Qty"; integer)
        {
            Caption = 'Archive Quote Qty';
            FieldClass = FlowField;
            CalcFormula = count("Sales Line Archive" where("Document Type" = filter(Quote), type = filter(Resource), "no." = field("no."), "Shipment Date" = field("date filter")));
            //FieldPropertyName = FieldPropertyValue;
        }
        field(50104; "BAL Archive Order Qty"; integer)
        {
            Caption = 'Archive Order Qty';
            FieldClass = FlowField;
            CalcFormula = count("Sales Line Archive" where("Document Type" = filter(Order), type = filter(Resource), "no." = field("no."), "Shipment Date" = field("date filter")));
            //FieldPropertyName = FieldPropertyValue;
        }
    }
    var
